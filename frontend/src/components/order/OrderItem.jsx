...
import { useGetOrderDetailsQuery } from "../../store/orderSlice";

const { Option } = Select;

const OrderItem = ({ item, onPriceUpdate, isDeliver }) => {
  const [newPrice, setNewPrice] = useState("");
  const [supplier, setNewSupplier] = useState("");

  const dispatch = useDispatch();
  const { data: orderDetails, refetch: refetchOrderDetails } =
    useGetOrderDetailsQuery({ orderId: item.orderId });
  ...
  const handlePriceUpdate = async () => {
    try {
      ...
      const response = await axios.put(
        `/api/orders/updateItemPrice/${item._id}`,
        {
      ...
      if (response.status === 200) {
        ...
        onPriceUpdate(item._id, updatedOrderItem);
  ...
  return (
    <tr key={item._id} className="">
      ...
      <td>
        {item?.product?.supplier ? item?.product?.supplier : "N/A"}
      </td>
